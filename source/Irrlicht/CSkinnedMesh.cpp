...
// TODO this has the potential to break subsecond animations, doesn't it?
...
void CSkinnedMesh::buildAllLocalAnimatedMatrices()
{
	for (u32 i=0; i<AllJoints.size(); ++i)
	{
			 ...
			 joint->UseAnimationFrom->RotationKeys.size() ))
		{
			...
			// TODO ah btw this might break .b3d and .x but I don't care for now
			
			core::matrix4 transMat;
			transMat.setTranslation(joint->Animatedposition);

			core::matrix4 rotMat;
			joint->Animatedrotation.getMatrix_transposed(rotMat);
			...
			joint->LocalAnimatedMatrix =  transMat * rotMat * scaleMatrix;

			// TODO this does not look like TRS; it seems to be RTS / STR?
...
void CSkinnedMesh::skinJoint(SJoint *joint, SJoint *parentJoint)
{
	if (joint->Weights.size())
	{
		...
		core::matrix4 jointVertexPull = joint->GlobalAnimatedMatrix * joint->GlobalInversedMatrix;
		...
		for (u32 i=0; i<joint->Weights.size(); ++i)
		{
			...
			// TODO this needs to take scale into account
			if (AnimateNormals)
